...
<template>
  ...
  >
    <div class="mb-3 flex items-center justify-between gap-2">
      <span class="inline-flex items-center gap-1 text-xs font-medium" :class="CATEGORIES[post.category].color">
    ...
    <h2 class="font-display text-lg font-semibold text-heading transition group-hover:text-accent-soft">
      {{ post.title }}
    </h2>
    ...
    <ul v-if="post.tags?.length" class="mt-4 flex flex-wrap gap-1.5">
      <li v-for="tag in post.tags.slice(0, 3)" :key="tag" class="tag-pill rounded-md px-2 py-0.5 text-xs">
        #{{ tag }}
      </li>
    </ul>
  </RouterLink>
